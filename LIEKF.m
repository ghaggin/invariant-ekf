...
%% Left-Invariant filter class, predicts next state, corrects prediction
    ...
    methods
        ...
        function prediction(obj, w, a, dt)  %TBC bias
            ...
            obj.bias_pred = zeros(6,1);
            obj.Sigma_pred = phi*obj.Sigma*(phi') + phi*obj.Q*(phi')*dt;
        ...
        function correction(obj,GPS)    %GPS 3X1
            ...
            K = obj.Sigma_pred * H' / S;     % Kalman gain
            K_X = K(1:9,:);
            K_B = K(10:15,:);
            PI = [eye(3), zeros(3,2)];
            
            nu = eye(5)/obj.mu_pred * Y;        % Innovation
            delta_X = K_X * PI * nu;         
            delta_B = K_B * PI * nu;         
            ...
            obj.Sigma = (eye(15)- K * H) * obj.Sigma_pred * (eye(15)- K * H)' + K * N * K';
        end
